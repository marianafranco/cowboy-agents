...
public class position_to_cluster extends DefaultInternalAction {
	...
	public Object execute(TransitionSystem ts, Unifier un, Term[] args) throws Exception{
		...
		if(args[0].isNumeric() && args[1].isNumeric() && args[2].isNumeric()){
			...
			//int dist = centers[0].getLocation(model).maxBorder(new Location(x,y));
			int dist2;
			for(int i = 1;i<centers.length;i++){
				dist2 = centers[i].getLocation(model).distanceChebyshev(new Location(x,y));
				//dist2 = centers[i].getLocation(model).maxBorder(new Location(x,y));
				if(dist2<dist){
					dist = dist2;
					n = i;
				}
...
//				&& model.getCorral().center().distanceChebyshev(from.getLocation(model))>8){
//			temp = Search.getNodeLocation(path.get(n-8));
	//	} else 
		if(n > model.getCorral().center().distanceChebyshev(from.getLocation(model)) 
