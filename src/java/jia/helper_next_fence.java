...
public class helper_next_fence extends DefaultInternalAction {
	...
	public static int last_x=0, last_y=0;
    ...
    public Object execute(TransitionSystem ts, Unifier un, Term[] terms) throws Exception {
        try {
            ...
            if (agent.getFences().size() != 0) {
                ...
                // desired position
                int itox = next_fence.cheatHelperX();
                int itoy = next_fence.cheatHelperY();
                
                if (itox == iagx && itoy == iagy) {
                	CowboyArch.setHelper_in_position(true);
                	if (CowboyArch.isCheat_passed())
                    	agent.getFences().remove(0);
                }
                else
                	CowboyArch.setHelper_in_position(false);
                
                // workaround to not get stucked in obstacles
                if (!CowboyArch.isHelper_in_position() && System.currentTimeMillis() - last_time_pos_changed > 10000) {
