...
public class SearchOld {
    ...
    SearchOld(LocalWorldModel m, Location from, Location to, int[] actions, boolean considerFenceAsObstacles) {
        this.model = m;
        this.from  = from;
        this.to    = to;
    ...
    SearchOld(LocalWorldModel m, Location from, Location to) {
        this(m,from,to,null,false);
    }

    public Nodo search() throws Exception { 
        Busca searchAlg = new AEstrela();
        GridStateOld root = new GridStateOld(from, "initial", this);
        root.setIsRoot();
...
final class GridStateOld implements Estado, Heuristica {
    ...
    public GridStateOld(Location l, String op, SearchOld ia) {
        this.pos = l;
        this.op  = op;
        this.ia  = ia;
    ...
    public List<Estado> sucessores() {
        ...
        if (isRoot) {
            Collections.sort(s, new VisitedComparatorOld(ia.model));
        }
        return s;
    }

    private void suc(List<Estado> s, Location newl, String op) {
        if (ia.model.isFree(newl) && !ia.model.hasObject(WorldModel.COW, newl) && !ia.model.hasObject(WorldModel.ENEMY, newl)) {
        	if (ia.considerFenceAsObstacles && ia.model.hasObject(WorldModel.CLOSED_FENCE, newl.x, newl.y)) {
        		return;
        	}
        	s.add(new GridStateOld(newl,op,ia));
        }
    }

    public boolean equals(Object o) {
        if (o != null && o instanceof GridStateOld) {
            GridStateOld m = (GridStateOld)o;
...
class VisitedComparatorOld implements Comparator<Estado> {

    LocalWorldModel model;
    VisitedComparatorOld(LocalWorldModel m) {
        model = m;
    }

    public int compare(Estado o1, Estado o2) {
        int v1 = model.getVisited(((GridStateOld)o1).pos);
        int v2 = model.getVisited(((GridStateOld)o2).pos);
        if (v1 > v2) return 1;
        if (v2 > v1) return -1;
        return 0;
    }
